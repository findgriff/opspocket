...
# BUG #8-fix: detect an existing multi-site Caddy setup and PRESERVE it.
# Previously this script clobbered /etc/caddy/Caddyfile unconditionally
# — on a shared host (e.g. our dev box) that wipes out every other site.
#
# If the main Caddyfile contains an `import Caddyfile.d/*.caddy` directive,
# we install ourselves as an additional site block at
# /etc/caddy/Caddyfile.d/<domain>.caddy instead of rewriting the main file.
# Otherwise (fresh single-site install) we write the main Caddyfile as before.
#
...
# Render the site block. In multi-site mode it gets its own file under
# Caddyfile.d/ and the main Caddyfile is left alone. In single-site mode
# it becomes the whole Caddyfile.
SITE_BLOCK=$(cat <<CADDY
...
if [[ "$MULTI_SITE" == "1" ]]; then
  ...
  # Derive a safe filename from the domain
  SAFE_NAME=$(echo "$DOMAIN" | tr '.' '-')
  printf '%s\n' "$SITE_BLOCK" > "/etc/caddy/Caddyfile.d/${SAFE_NAME}.caddy"
  say "Installed site block at /etc/caddy/Caddyfile.d/${SAFE_NAME}.caddy (multi-site mode preserved)"
else
